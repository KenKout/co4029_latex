...
\begin{enumerate}
  \item \textbf{Role-based clarity} — students, instructors and administrators see distinct navigation structures and role-appropriate destinations, preventing cognitive overload from features that are not relevant to the active role.
  ...
  \item \textbf{Responsive shell architecture} — a persistent \path|AppShell| component renders a desktop sidebar that collapses from 256\,px (\path|w-64|) to 64\,px (\path|w-16|, icons only) by user toggle. On narrow screens the sidebar is replaced by an off-canvas navigation menu opened from the mobile top bar.
\end{enumerate}
...
\begin{longtable}{|p{0.22\textwidth}|p{0.20\textwidth}|p{0.46\textwidth}|}
  ...
  \hline
  Minimum size & 12\,px for body and control text; 10\,px permitted for compact metadata and status badges & No primary content or interactive control label falls below 12\,px; compact, non-primary metadata may use a smaller label size when its meaning is also conveyed by surrounding text or structure. \\
  \hline
\end{longtable}
...
The lesson editor (\path|/teacher/courses/$courseId/lessons/$lessonId|) is a
long scrolling page, so it is shown in two parts: the authoring area and its
settings sidebar in Figure~\ref{fig:instructor_lesson_manage}, and the material
and collaboration sections that follow it in
Figure~\ref{fig:instructor_lesson_manage_lower}. The two overlap by one section
heading so the join is readable.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth,height=0.42\textheight,keepaspectratio]{graphics/screenshots/instructor_lesson_manage_a.png}
  \caption{Lesson Editor (upper) --- Markdown content editor with the right-hand lesson settings and prerequisites panel.}
  \label{fig:instructor_lesson_manage}
\end{figure}

The upper half is split into a content area and a settings sidebar. The left
side shows a Markdown editor for reading lessons, with bold, italic, list,
heading, link, image and code controls and a Write / Preview toggle. The right
sidebar exposes Estimated Duration, the completion rules that duration implies,
Difficulty Level, and a Prerequisites picker. A persistent \textbf{Save
changes} button anchors the top-right corner alongside the Archive, Delete and
Unpublish actions, and a banner above the title offers to generate a draft quiz
from the material already attached to the lesson.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth,height=0.78\textheight,keepaspectratio]{graphics/screenshots/instructor_lesson_manage_b.png}
  \caption{Lesson Editor (lower) --- downloadable resources, material processing history, the curated knowledge graph, and lesson discussion.}
  \label{fig:instructor_lesson_manage_lower}
\end{figure}

The lower half carries everything downstream of the lesson text. A
\textit{Downloadable Resources} block holds attached PDFs and other files, each
with a \textit{Use for AI} switch deciding whether the document also enters the
ingestion pipeline for quizzes, search and the knowledge graph. \textit{Material
history} reports the processing state of each version --- the \texttt{Ready}
badge here is what Section~\ref{sec:impl_ingestion} describes the worker
writing. The \textit{Knowledge Graph} panel renders the curated concept graph
for the lesson with its own publish toggle, showing the top concepts of the
extracted set, and \textit{Discussion} carries the lesson's threads. Destructive
actions are kept at the top bar rather than beside the content to reduce
mis-clicks.
...
\begin{longtable}{|p{0.22\textwidth}|p{0.20\textwidth}|p{0.46\textwidth}|}
  ...
  \hline
  \path|SideNavBar| & Authenticated (all) & Always rendered on desktop. It collapses to \path|w-16| (icons only) or expands to \path|w-64| by user toggle; narrow screens use an off-canvas navigation menu opened from the mobile top bar. Receives a role-specific \path|navItems| array as a prop. \\
  \hline
...
The \path|AppShell| component is the structural keystone of the authenticated experience. The shell itself is role-agnostic; role differentiation is communicated through the \path|navItems| prop (defined in \path|src/lib/navigation.ts|) and through shared design tokens. On desktop the sidebar can collapse to an icon rail; on narrow screens it is replaced by a mobile top-bar menu, keeping the main column readable without permanently consuming horizontal space.
