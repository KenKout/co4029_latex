...
\textbf{Description} & Allows Students to participate in unified hybrid mock interview sessions supporting typed and spoken turns, while the system records transcript, metadata, and session activity signals. Adaptive sequencing and follow-ups apply only when the relevant rollout flags are enabled; otherwise the session uses the established legacy progression path. \\ \hline
...
2. The system starts a hybrid room that supports typed turns and optional microphone interaction. \newline
3. The system applies adaptive sequencing and follow-ups only when the relevant rollout flags are enabled; otherwise it uses the legacy progression path. \newline
4. The AI interviewer presents a question. \newline
5. The Student responds by typing or speaking. \newline
6. When spoken interaction is available, the runtime streams audio, transcribes speech, and generates text-to-speech responses. \newline
7. Voice-answer media may be linked to stored audio; optional whole-session recording requires current-policy consent and deployment enablement, and a recording failure does not interrupt the interview. \newline
...
10. The system records browser activity signals such as focus changes, tab switching, full-screen status, and session events for review. \\ \hline
\textbf{Alternative Flows} &
