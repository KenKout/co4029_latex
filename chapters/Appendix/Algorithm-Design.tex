...
is unusual enough that a linear description of it misleads. Each of the four
--- card review scheduling, career-pathway stage evaluation, material
ingestion, and quiz generation --- crosses several processes, branches on
conditions that are not obvious from the endpoint that starts it, and has at
least one path that exists only to handle a failure. The diagrams show the
branches and the failure paths together, which prose in sequence cannot.
