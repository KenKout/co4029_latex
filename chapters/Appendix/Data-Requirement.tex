...
system must record each outcome against its interview configuration,
which belongs to a course module, together with its ordering within the
outcome set. Outcomes are the authoritative reference against which
post-session assessment is performed.
...
pass verdict, cooldown and time-limit rules, and any supplementary
instructions provided to the AI assessment model. An interview configuration is course-scoped; it may be surfaced within a module's ordered learning journey but is not owned by a lesson or module record.

% ─────────────────────────────────────────────────────────────────────────────
\subsection{Student Interview Sessions}
Each student interview session is an independent interaction between
the student and the interview runtime. The system records the session's
association with a specific interview configuration, the student's
identity, the attempt number, hybrid input mode, onboarding and terminal
state, and start and end timestamps.

The system stores the full session transcript as structured turns and
links it to the specific questions drawn from the question bank during
that session, along with the student's response to each. Voice-answer
media may be represented by stored object references; optional
whole-session audio recording is separately consented, feature-gated,
and retained under its own lifecycle.
...
exposed to the student. After successful evaluation, the student-facing
result provides a binary pass or fail together with qualitative
remediation and a study plan, while withholding numeric rubric scores,
per-outcome verdicts, and internal reasoning.

The system must additionally maintain the \textit{runtime state} of an
in-progress native interview as a durable record rather than in
memory alone: the current question and phase, provisional per-outcome
coverage, the ledgers of questions asked and skipped, follow-up and
hint counters, and the most recent runtime analysis. This record
carries a monotonic version counter. Typed turns additionally have a
durable receipt and idempotency key, so a retransmission can be
acknowledged without applying a second fold. Transcript rows retain
the question linkage needed by later reconciliation and terminal
evaluation. The native conversational context itself is ephemeral;
PostgreSQL runtime state and messages remain the authority. Provisional
coverage is later reconciled and is distinct from the final outcome
verdict and pass decision.
...
The server-scored browser-integrity policy under which an attempt is
judged is \textit{frozen onto the attempt} when it begins: the signal
weights and flag threshold cannot be changed by a later configuration
edit. Interview delivery also uses a mandatory frontend-only local
camera gate. It checks for an active video track and blocks the
workspace when absent, but neither the interview configuration nor the
session policy snapshot stores a camera requirement, and no camera
media is uploaded or retained.
