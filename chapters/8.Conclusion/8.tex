...
\begin{enumerate}
    ...
    \item \textbf{Multi-tenant Organisation Management} --- Tenant provisioning, organisational-unit hierarchies rooted at faculties, staff affiliation, membership lifecycle, domain attachment with optional auto-provisioning, and bulk onboarding by CSV with per-row validation that reports failures without aborting the batch. One account belongs to one organisation, enforced by a partial unique index rather than by convention alone.
