...
The metrics above govern movement \textit{within} a course. A career pathway is the structure above it: an ordered sequence of stages, each bundling required courses with a set of electives from which the stage asks for a stated number. Progress along a pathway, and the decision that a student has finished one, are therefore computed over stages rather than over cards.
...
\paragraph{Completion Is a Predicate, Not a Threshold}
...
and not by testing $P \geq 100$. The distinction is not cosmetic. The platform retains an earlier progress formula — a flat mean of every attached course's completion percentage — selected by a deployment-wide setting, so that an installation may migrate on a dated cutover rather than have its reported figures move beneath it. Under that earlier formula a pathway whose required courses and elective quota are all satisfied still scores below $100$ while surplus electives sit untouched, and a completion gated on the percentage would never fire at all. Deriving completion from the stages makes it independent of whichever formula is presenting the number.

Since $d_s \leq t_s$ for every stage, the predicate and the aggregate agree exactly wherever the stage-aware formula is the one in force: $P = 100$ if and only if every stage is complete. The two are one rule under two presentations, which is what prevents a pathway reading as finished on screen while the records built on completion remain open.
