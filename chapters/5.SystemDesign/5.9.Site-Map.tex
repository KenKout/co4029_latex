...
Figure~\ref{fig:sitemap} summarises the full route tree, organised by the five role groups exposed by the platform: \textit{Public}, \textit{Student}, \textit{Instructor}, \textit{Educational Manager}, and \textit{Administrator}. The router declares 96 paths in total: 80 render a destination page, and the remaining 16 are redirect-only aliases retained so that previously published links keep resolving. The complete per-route URL listing --- with page name, dynamic parameters and purpose --- is provided in Appendix~\ref{appendix:sitemap}.
