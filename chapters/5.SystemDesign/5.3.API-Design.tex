...
Rather than being enumerated here, the surface is summarised by top-level path prefix in Table~\ref{tab:api_surface_by_prefix} and catalogued per operation --- method, path, request and response shape --- in Appendix~\ref{appendix:api-design}.

Both the table below and the appendix catalogue are \emph{generated} from the application's own OpenAPI schema by \path|scripts/gen_api_tables.py|, not maintained by hand. This is a deliberate response to how the two drifted apart: a surface this size changes every release, and a count that has to be retyped is a count that silently goes stale. Regenerating is one command, and the output cannot contradict the code it was read from.

The distribution itself is the design point. Roughly two thirds of all operations sit under three prefixes --- \path|/teacher|, \path|/admin| and \path|/management| --- because authoring, platform operations and programme governance are where the system's complexity lives. The learner-facing surface (\path|/me|, \path|/courses|, \path|/attempts|) is deliberately narrow: a student's interaction is guided by the curriculum rather than assembled from arbitrary queries, so it needs few endpoints and each does one thing.

\input{generated/api_surface_by_prefix}
