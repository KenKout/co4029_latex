...
The class diagram captures the static structure of the backend: the core domain entities, their attributes, and the relationships between them. The model is organised along Domain-Driven Design lines into four packages --- \textit{Identity \& Access}, \textit{Course \& Ingestion}, \textit{Assessment \& Interview}, and \textit{Programme \& Path Governance}. The four paragraphs below state the design decision that shapes each package; the full entity listing, with attributes, cardinalities and the rationale behind each relationship, is catalogued in Appendix~\ref{appendix:class-diagram}.
...
\begin{itemize}
    ...
    \item \textbf{Programme \& Path Governance.} A student's career pathway is held as an \textbf{attempt record, not as a field on the enrolment}. \texttt{ProgramEnrollment} owns many \texttt{ProgramPathAttempt}s, each pinned to a career-path \textit{version} and carrying how it began --- assigned from the programme default, chosen by the student, or produced by an approved change. A single field could not express a student holding two pathways at once, nor survive one ending, and the closed attempt is what the audit is written against. \texttt{PathChangeRequest} makes leaving a pathway reviewable: one open request per enrolment, discriminated by \texttt{kind} into a change or a drop.
\end{itemize}
