...
Two properties hold across the set and are worth stating once rather than repeating in each requirement. Every artefact an AI pipeline generates enters a review state and requires a human decision before a learner can see it. And every rule that judges a learner --- the integrity policy on an attempt, the pathway version on an enrolment, the programme version on a student --- is frozen at the moment it starts applying to them, so that editing the curriculum never retroactively changes what somebody already under way is measured against.
