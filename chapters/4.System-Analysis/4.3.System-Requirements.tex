...
\begin{itemize}
    ...
    \item \textbf{UR-ITA-04:} As an IT Administrator, I want every action I take inside a tenant --- restoring deleted content, disabling an account, amending an organisation's configuration --- to be recorded in the immutable audit trail, so that platform authority broad enough to recover a tenant from an operational failure remains accountable, and pedagogical decisions stay in practice with the Educational Manager and Teachers even where the permission model does not forbid me from making them.
