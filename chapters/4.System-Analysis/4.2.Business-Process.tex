...
\begin{enumerate}
    ...
    organisation-configurable ceiling on concurrent enrolments, and
    each enrolled student pins exactly one career pathway. Enrolment is
    what grants access to the courses that Stages~2 through~4 operate
    upon; without it a student has no curriculum and the loop cannot
    begin.
    ...
    may request a change of career pathway with a written
    justification; the Faculty Dean who owns the programme
    ...
    An approval snapshots the progress earned on the outgoing pathway
    and preserves completed courses as awards that carry across, so
    that work shared between two pathways is credited once and never
    repeated. The decision, and the reason behind it, are returned to
    the student.
