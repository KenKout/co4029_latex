...
\diagramplaceholder{fig:impl_auth_flow}{Authentication and session lifecycle}{A flow chart of both sign-in routes --- password and Google OAuth --- converging on session issue, with the MFA gate as a required second step before an access token is released. Show refresh-token rotation, and the revocation paths (explicit logout, account disable) that terminate every live session.}
