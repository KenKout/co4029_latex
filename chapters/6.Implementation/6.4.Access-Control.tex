...
\diagramplaceholder{fig:impl_permission_resolution}{Permission resolution for one request}{A flow chart resolving a caller's effective permissions: role assignments across organisation memberships $\rightarrow$ expansion of each role via the permission catalog $\rightarrow$ merge of direct grants $\rightarrow$ Redis cache $\rightarrow$ the guard's containment check. Show the recursive organisation-unit walk that determines which scopes contain the target resource.}
