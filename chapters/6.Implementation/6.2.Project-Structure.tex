...
|   +-- discussions/      # Lesson and course discussion topics and comments
...
|   +-- learning_programs/# Programme authoring, enrollment, path governance
|   +-- materials/        # Learning material upload, ingestion pipeline
|   +-- notifications/    # In-app and email notification dispatch
|   +-- policies/         # Versioned legal and academic policy documents
...
Schema evolution is managed by 121 Alembic migration files running from \texttt{0001} to \texttt{0122}. Migration \texttt{0001} establishes the full baseline schema ported from a hand-managed SQL file. Subsequent migrations add partial unique indexes, cascade rules, audit columns, pgvector half-precision embeddings (\texttt{halfvec(3072)}), full-text search vectors, and the append-only trigger that protects the audit log. Every migration is reversible via a \texttt{downgrade()} function; where reversal would destroy records that cannot be reconstructed, the downgrade refuses rather than proceeding silently.
