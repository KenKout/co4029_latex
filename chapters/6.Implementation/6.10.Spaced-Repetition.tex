...
Taken together the pieces form a closed cycle. A graded answer yields $Q$; $Q$ drives the ease-factor update; the updated ease factor and the repetition count give the next interval; jitter spreads that interval so a cohort enrolled on one day does not return on one day; and the resulting \texttt{due\_at} is what the background scan later reads to raise a reminder. An incorrect answer short-circuits the middle of that chain: a grade below three resets the interval to a single day regardless of the ease factor, so a lapse returns the card to the front of the queue rather than merely shortening its next gap. The card state written at the end of each pass is also the state the unlock query reads, which is why unlock is evaluated after the write rather than alongside it --- the two must never disagree about what the learner has demonstrated.
