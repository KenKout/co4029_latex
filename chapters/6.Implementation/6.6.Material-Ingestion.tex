...
\diagramplaceholder{fig:impl_ingestion_pipeline}{Material ingestion pipeline and processing states}{Show one uploaded material version moving through extraction, chunking, embedding and indexing, with the processing status at each stage and the failure edge. Mark the point at which the object store holds the bytes and the database holds only the reference, and show that the previous version's chunks stay queryable until the new version completes.}
