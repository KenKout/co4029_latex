...
\diagramplaceholder{fig:impl_hybrid_retrieval}{Hybrid retrieval and generation pipeline}{A flow chart of a generation request: parallel dense (pgvector) and sparse (BM25) retrieval, reciprocal-rank fusion, MMR diversification, and the role filter that removes chunks the requester may not access, feeding the LLM gateway. Show the human review gate that every generated item must pass before reaching a learner.}
