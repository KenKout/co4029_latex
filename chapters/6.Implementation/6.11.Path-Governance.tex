...
\diagramplaceholder{fig:impl_path_request_states}{Path request review lifecycle}{A state machine over \texttt{pending}, \texttt{in\_progress}, \texttt{approved}, \texttt{rejected}, \texttt{cancelled} and \texttt{invalidated}, shared by change and drop requests. Show the divergent approval effects --- a change opens a replacement attempt, a drop opens none --- and mark the invariants re-checked at the decision rather than trusted from filing.}
