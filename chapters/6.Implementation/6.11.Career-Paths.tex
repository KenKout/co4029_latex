...
Satisfaction, stage completion, unlocking and progress are computed in exactly one place, \path{services/stages.py}. The learner's progress endpoint, the manager's roster, the readiness worker and the Start endpoint all call into it rather than deriving any part for themselves. The discipline is not stylistic: the subsystem has twice shipped a defect of exactly this shape --- a second implementation of ``finished'' that was individually reasonable and disagreed with the first at the margin, with nothing watching for the disagreement. A rule with one implementation cannot drift from itself.
