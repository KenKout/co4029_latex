...
This chapter evaluates the first iteration of the aBridgeAI system across four dimensions: frontend user experience (Section~\ref{sec:eval_frontend}), backend performance under load (Section~\ref{sec:eval_backend}), functional-requirement coverage (Section~\ref{sec:eval_fr_coverage}), and AI model comparison for quiz generation (Section~\ref{sec:eval_ai_model}).
...
Both audited pages achieve scores above 95 in all Lighthouse categories. This automated evidence supports the audited frontend configuration and web-platform practices, but does not independently establish WCAG~2.1 AA conformance without complementary keyboard, contrast, and assistive-technology checks.
...
\begin{itemize}
    \item \textbf{Measured HTTP scenarios:} The three read-only LAN scenarios sustained 100 virtual users with 0\% error rate. Median response times remained below 215\,ms and p95 latency remained below 510\,ms for those scenarios.
    \item \textbf{NFR-2.1 status: partially evidenced, not fully verified.} The measured scenarios do not include the NFR's concurrent AI-interview sessions, video streams, write paths, worker contention, TLS termination, or LiveKit media delivery. They therefore support backend read-path capacity but cannot establish full end-to-end satisfaction of NFR-2.1.
    \item \textbf{Throughput and bottleneck:} The measured scenarios sustain 136--431 requests per second depending on query complexity. Similar course-catalog and quiz-operation latency profiles (p50 $\approx$ 210\,ms) suggest that the database query layer, rather than framework overhead (health-check p50 = 135\,ms), is the main contributor within this limited benchmark.
\end{itemize}

% ===========================================================
\subsection{Requirements Scope}
\label{sec:eval_fr_coverage}

This section distinguishes the Functional Requirements Specification (FRS) prepared for the Specialized Project phase from the final Capstone Project FRS. The Specialized Project FRS contains 57 functional requirements (FRs), represented by Modules 1--4 in Appendix~\ref{appendix:fr}. The Capstone Project FRS expands the specification to 106 FRs. Its additional Bonus section contains 49 FRs in Modules 5--8.

Table~\ref{tab:fr_scope_comparison} compares the two requirements baselines. The increase reflects broader capstone scope.

\begin{table}[htbp]
\centering
\caption{Functional Requirements Scope: Specialized Project and Capstone Project}
\label{tab:fr_scope_comparison}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{|l|c|p{0.50\textwidth}|}
\hline
\textbf{Requirements baseline} & \textbf{FR count} & \textbf{Interpretation} \\
\hline
Specialized Project FRS & 57 & Requirements in Modules 1--4 of Appendix~\ref{appendix:fr}. \\
\hline
Bonus & 49 & Additional requirements in Modules 5--8 of Appendix~\ref{appendix:fr}. \\
\hline
Capstone Project FRS & 106 & Full final specification: Specialized Project FRS plus Bonus requirements. \\
\hline
\end{tabular}
\end{table}
